...
function defaultItemForm() {
  return {
    ...
    timePricingText: JSON.stringify(
      [{ name: '早餐時段 9 折', start: '06:00', end: '10:30', days: [1, 2, 3, 4, 5, 6, 0], price: 0 }],
      null,
      2
    )
  };
...
function parseTimePricing(text, basePrice) {
  if (!text.trim()) {
    return [];
  }
...
export default function MenuPage() {
  ...
  const submitItem = () => {
    const payload = {
      ...
      timePricing: parseTimePricing(itemForm.timePricingText, Number(itemForm.basePrice))
    };
      ...
      <section className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
        <article className="admin-panel p-5">
          <div className="flex items-center justify-between gap-3">
            ...
            <button type="button" className="admin-ghost" onClick={() => setCategoryForm(defaultCategoryForm())}>新增分類</button>
          </div>
          ...
          <div className="mt-4 grid gap-3">
            ...
            <input className="admin-field" type="number" placeholder="排序" value={categoryForm.sortOrder} onChange={(event) => setCategoryForm((current) => ({ ...current, sortOrder: Number(event.target.value) }))} />
            <label className="rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-600">
              <input checked={categoryForm.isActive} onChange={(event) => setCategoryForm((current) => ({ ...current, isActive: event.target.checked }))} type="checkbox" /> 啟用此分類
            </label>
            <button type="button" className="admin-button" onClick={() => categoryMutation.mutate(categoryForm)}>儲存分類</button>
          </div>
            ...
            {categories.map((category) => (
              <div key={category.id} className="rounded-2xl bg-slate-50 px-4 py-3">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <button type="button" className="admin-ghost" onClick={() => setCategoryForm(category)}>編輯</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <article className="admin-panel p-5">
          <div className="flex items-center justify-between gap-3">
            ...
            <button type="button" className="admin-ghost" onClick={() => setItemForm(defaultItemForm())}>新增品項</button>
          </div>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            ...
            <input className="admin-field" placeholder="Emoji" value={itemForm.emoji} onChange={(event) => setItemForm((current) => ({ ...current, emoji: event.target.value }))} />
            <input className="admin-field" placeholder="圖片網址" value={itemForm.imageUrl} onChange={(event) => setItemForm((current) => ({ ...current, imageUrl: event.target.value }))} />
            <textarea className="admin-field min-h-24 resize-none md:col-span-2" placeholder="品項說明" value={itemForm.description} onChange={(event) => setItemForm((current) => ({ ...current, description: event.target.value }))} />
            <textarea className="admin-field min-h-36 resize-none md:col-span-2" placeholder="時段定價 JSON" value={itemForm.timePricingText} onChange={(event) => setItemForm((current) => ({ ...current, timePricingText: event.target.value }))} />
          </div>

          <div className="mt-4 rounded-2xl border border-slate-200 p-4">
            <div className="text-sm font-semibold text-slate-700">可套用加料群組</div>
          ...
          <button type="button" className="admin-button mt-4" onClick={submitItem}>儲存品項</button>
        </article>
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
        <article className="admin-panel overflow-hidden">
          <div className="flex items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
            <h2 className="text-xl font-bold text-slate-900">菜單清單</h2>
            <div className="flex gap-3">
              <button type="button" className="admin-ghost" onClick={() => exportMutation.mutate()}>匯出 JSON</button>
            </div>
          </div>
                  ...
                  <tr key={item.id} className="border-t border-slate-100">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        <button type="button" className="admin-ghost" onClick={() => setItemForm({
                          id: item.id,
                          name: item.name,
        ...
        <article className="admin-panel p-5">
          <div className="flex items-center justify-between gap-3">
            ...
            <button type="button" className="admin-ghost" onClick={() => setAddonForm(defaultAddonForm())}>新增群組</button>
          </div>

          <div className="mt-4 space-y-3">
            <input className="admin-field" placeholder="群組名稱" value={addonForm.name} onChange={(event) => setAddonForm((current) => ({ ...current, name: event.target.value }))} />
            <input className="admin-field" type="number" placeholder="最多可選" value={addonForm.maxSelect} onChange={(event) => setAddonForm((current) => ({ ...current, maxSelect: Number(event.target.value) }))} />
            <label className="rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-600">
              <input checked={addonForm.required} onChange={(event) => setAddonForm((current) => ({ ...current, required: event.target.checked }))} type="checkbox" /> 此群組為必選
            </label>
            {addonForm.options.map((option, index) => (
              <div key={`${index}-${option.name}`} className="grid gap-3 md:grid-cols-[1fr_140px]">
                <input className="admin-field" placeholder="選項名稱" value={option.name} onChange={(event) => setAddonForm((current) => ({
                  ...current,
                  options: current.options.map((entry, optionIndex) => optionIndex === index ? { ...entry, name: event.target.value } : entry)
                }))} />
                <input className="admin-field" type="number" placeholder="加價" value={option.price} onChange={(event) => setAddonForm((current) => ({
                  ...current,
                  options: current.options.map((entry, optionIndex) => optionIndex === index ? { ...entry, price: Number(event.target.value || 0) } : entry)
                }))} />
              </div>
            ))}
            <button type="button" className="admin-ghost" onClick={() => setAddonForm((current) => ({ ...current, options: [...current.options, { name: '', price: 0 }] }))}>新增選項</button>
            <button type="button" className="admin-button" onClick={() => addonMutation.mutate(addonForm)}>儲存加料群組</button>
          </div>
              ...
              <div key={group.id} className="rounded-2xl bg-slate-50 p-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-1 text-sm text-slate-500">{group.options.map((option) => `${option.name}${option.price ? ` (+${option.price})` : ''}`).join('、')}</div>
                  </div>
                  <button type="button" className="admin-ghost" onClick={() => setAddonForm(group)}>編輯</button>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mt-8 border-t border-slate-100 pt-6">
            ...
            <p className="mt-2 text-sm leading-7 text-slate-500">
              可貼上先前匯出的 JSON，或選擇本機檔案。勾選「覆蓋現有菜單」時，系統會先停用既有資料再匯入新菜單。
            </p>
            <textarea className="admin-field mt-4 min-h-40 resize-none" placeholder="貼上菜單 JSON" value={importPayload} onChange={(event) => setImportPayload(event.target.value)} />
            <input
            ...
            <label className="mt-3 block rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-600">
              <input checked={replaceAll} onChange={(event) => setReplaceAll(event.target.checked)} type="checkbox" /> 覆蓋現有菜單資料
            </label>
            <button
              type="button"
